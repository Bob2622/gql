...
export default {
   ...
   */
};export async function mergeFileGlob(fileGlob) {
 ...
 * @return {Promise<string>} A promise of the resulting string.
 */
export async function mergeFilePaths(filePaths) {
 ...
 */
export function mergeStrings(schemaStrs) {
 ...
 * @return {string} The resulting merged GraphQL string.
 */
export function mergeString(schemaStr) {
 ...
 * @return {string} The resulting merged GraphQL string.
 */
export function mergeAst(schemaAst) {
  const typeDefs = {};

  // Go through the AST and extract/merge type definitions.
  const editedAst = visit(schemaAst, {
    enter(node) {
...
export async function cli(program = commander) {
  if (!module.parent) {
    program.version(version).usage('[options] <glob ...>');
...
function cliAddBasics(command) {
  return command.description(description).option('-o, --out-file <path>', 'Output GraphQL file, otherwise use stdout');
}

function cliAddHelp(command) {
  const commandName = !module.parent ? 'gql-merge' : 'gql merge';
